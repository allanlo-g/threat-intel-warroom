...
| 檔案 | 對象 | 內容 |
|------|------|------|
| **`index.html`** | 一般 User | **唯讀**：只有「戰情看板 + 情資明細」，可看、可搜尋、可下載 CSV，**沒有任何編輯/維護功能** |
| **`maintenance.html`** | 維護者（顧問/客戶） | **完整**：另含情資維護、來源維護、年度設定、說明（即原本的完整看板） |

兩頁共用同一份 `assets/`，無重複程式碼。GitHub Pages 根網址 `/` 是唯讀版給一般 User；
維護者開 `/maintenance.html`。本機 `start.bat` 預設開維護後台。
...
### 本機執行
```
雙擊 start.bat            (Windows，預設開維護後台 maintenance.html)
# 或
python -m http.server 8080
#   唯讀看板： http://localhost:8080/index.html
#   維護後台： http://localhost:8080/maintenance.html
```
> 直接用 `file://` 雙擊 HTML 會被瀏覽器擋住讀取設定檔，請務必透過本機伺服器開啟。
...
> 部署後：根網址 `/`＝唯讀看板（一般 User）；維護者開 `/maintenance.html`。
> 若要公開某客戶獨立版，把 workflow 的 `path` 指向 `dist/<id>`。
...
## 7. 檔案結構
```
index.html              唯讀看板（一般 User：戰情看板 + 情資明細）
maintenance.html        完整維護後台（顧問/客戶：含維護/編輯/設定）
